...
namespace DeuceGear.UnitTests.EnumExtensions
{
    ...
    public partial class EnumsTestFixture
    {
        ...
        [Test]
        public void EnumsListIntEnumPerformanceTest()
        {
            // arrange
            ...
            {
                var values = (int[])Enum.GetValues(typeof(IntEnum));
            }
            stopwatch.Stop();
            ...
            for (int i = 0; i < testLength; i++)
            {
                var values = (int[])typeof(IntEnum).GetEnumValues();
            }
            stopwatch.Stop();
            ...
            for (int i = 0; i < testLength; i++)
            {
                var values = Enums.EnumList<IntEnum, int>();
            }
            stopwatch.Stop();
        ...
        [TestCase(100)]
        [TestCase(1000)]
        public void EnumsListIntEnumPerformanceTestExcludingTheFirstCall(int testLength)
        {
            // arrange
            var stopwatch = new Stopwatch();

            // act - 1 - Enum.GetValues
            var values1 = (int[])Enum.GetValues(typeof(IntEnum));
            stopwatch.Start();
            for (int i = 0; i < testLength; i++)
            {
                var values = (int[])Enum.GetValues(typeof(IntEnum));
            ...
            // act - 2 - typeof().GetEnumValues
            var values2 = (int[])typeof(IntEnum).GetEnumValues();
            stopwatch.Restart();
            for (int i = 0; i < testLength; i++)
            {
                var values = (int[])typeof(IntEnum).GetEnumValues();
            ...
            // act - 3 - deucegear
            var values3 = Enums.EnumList<IntEnum, int>();
            stopwatch.Restart();
            for (int i = 0; i < testLength; i++)
            {
                var values = Enums.EnumList<IntEnum, int>();
